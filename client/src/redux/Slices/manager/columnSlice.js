...
export const fetchColumns = createAsyncThunk(
  ...
  async (_, { getState }) => {
    try {
      const { managerColumns } = getState();
      const token = getToken();
      const response = await axios.get(
        `${API_BASE_URL}/api/manager/getColumns`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
          params: {
            filters: managerColumns.filters,
          },
        }
      );
...
const updateTaskPosition = async (taskId, data) => {
  ...
  try {
    const response = await axios.put(
      `${API_BASE_URL}/api/manager/updateTaskPosition/${taskId}`,
      data,
      {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  async (data) => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/api/manager/createColumn`,
        data,
        {
          headers: {
            Authorization: `Bearer ${token}`,
