...
export const fetchColumns = createAsyncThunk(
  ...
  async (_, { getState }) => {
    try {
      const { adminColumns } = getState();
      const token = getToken();
      const response = await axios.get(`${API_BASE_URL}/api/admin/getColumns`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
        params: {
          filters: adminColumns.filters,
        },
      });
...
const updateTaskPosition = async (taskId, data) => {
  ...
  try {
    const response = await axios.put(
      `${API_BASE_URL}/api/admin/updateTaskPosition/${taskId}`,
      data,
      {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  async (data) => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/api/admin/createColumn`,
        data,
        {
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    updateLocalTaskOrder(state, action) {
      ...
      if (destColumnName === "Completed") {
        ...
        if (incompleteDependencies.length > 0) {
          toast.error("Cannot move to Completed. Dependencies are not completed.")
          return
        } else {
